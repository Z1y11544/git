...
# 3.2 分类
# 3.2.1 内置模块
# random、time、os、logging。等等可以直接导入使用的模块
# 3.2.2 第三方模块(第三方库)
# 下载：cmd窗口输入pip install 模块名
# 3.2.3 自定义模块
# 自己写的模块,需要导入才能使用
# 注意:命名要遵循标识符规定以及变量的命名规范，
#     并且不要与内置模块重名，否则将导致模块功能无法使用
# 3.3 导入模块
# 3.3.1 方式一
# 语法
# 导入模块
...
# 3.3.2 方式二
# 语法:
# from 模块名 import 功能名   # 导入函数只需要函数名不要()
# 调用功能：
# 直接输入功能即可，不需要添加模块名
# 3.3.3 方式三
# 语法:from 模块名 import *
# 注意:不建议使用这种方式导入模块，因为可能会出现重名的情况，导致无法使用模块功能
# 3.3.4 as起别名
# 1. as给模块起别名
# 语法:import 模块名 as 别名
# 2. as给模块中的功能起别名
...
# 2.文件被其他程序导入(别人执行我):__name__ = 模块名

# 4.包
# 4.1 含义：就是项目结构中的文件夹/目录
# 4.2 与普通文件的区别:包是含有__init__.py的文件夹
# 4.3 作用:将有联系的模块放到同一个文件夹下，
#          有效避免模块名称冲突等问题，让结构更清晰。
# 4.4 新建包:在项目目录中创建一个文件夹，并在其中创建__init__.py文件。

# 4.5 import导入包时，首先执行__init__.py文件中的代码
# 不建议在init文件中写大量代码
# 导包方式1:
# import 包名.模块名
# 导包方式2:
# from 包名 import 模块名
# 4.6 不建议在init文件中编写过多代码,尽量保证init文件简洁
# 4.7 __all__:本质上是一个列表，列表里面的元素代表要导入的模块
# 实际上就是 * 里面的内容
# 作用：可以控制要引入的东西，
# 4.8 包的本质就是一个模块，包又可以包含包

# 十二--闭包与装饰器
# 1.递归函数
# 1.1 含义:
# 如果一个函数在内部不调用其他函数，而是调用它本身的函数，就称为递归函数
# 1.2 条件
#   1.必须有一个明确的结束条件----递归出口
#   2.每进行一次递归，问题的规模都要比上一次递归的规模小
#   3.相邻两次重复之间有紧密的联系
...
#     n = 10          # 外层函数的局部变量
#     def inner():    # 内层函数
#         print(n)    # 内层函数引用了外层函数的局部变量
#     return inner    # 外层函数的返回值是内层函数的函数名
# print(outer())      # 返回的是inner函数的内存地址
# 第一种调用方式
# outer()()
# 第二种调用方式
# ot = outer()        # 调用外函数
...
# 1.封装
# 面向对象的三大特性：封装、继承、多态
# 1.1 封装：指的是隐藏对象中一些不希望被外部访问到的属性和方法
# 1.2 隐藏属性(私有权限)
# 只允许再类的内部使用,无法通过对象访问
# 在属性名或者方法前面加上两个下划线__
class Person:
...
# 1.3 私有属性/方法
# 一、xxx:普通属性/方法,如果是类中定义的,
# 二、__xxx:双下划线开头,隐藏属性,如果定义在类中
#           无法在外部直接访问,子类不会继承
